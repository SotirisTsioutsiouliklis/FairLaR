...
def new_index_set(p, index):
    """ Retunrs index from file.
    """
    n = len(p)
    sorted_index = np.argsort(-p)
    temp = np.loadtxt('targeted_set.txt', dtype=int)
    k = temp.size
    for i in range(k):
        sorted_index[i] = temp[i]
...
def fairPR(M, index, phi, k):
    ...
    index_top_k_1, index_top_k = new_index_set(p, index)
    n = p.size
...
with open("out_targeted_topk.txt", "w") as file_one:
    for i in fp:
        file_one.write(str(i) + "\n")